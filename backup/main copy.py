...
# Lista de objetos para incluir explicitamente no mapa
PLANETS = [
...
def gerar_mapa_astral(req: MapaAstralRequest):
    try:
        ...
        # Formato para Flatlib
        date = Datetime(utc_dt.strftime("%Y/%m/%d"), utc_dt.strftime("%H:%M"), "+00:00")
        pos = GeoPos(lat, lon)

        # Criação do mapa com sistema de casas iguais e IDs dos planetas
        ...
        # Coleta os planetas e pontos com grau e signo
        planetas = {
            obj.lower(): {
                "signo": chart.get(obj).sign,
                "grau": round(chart.get(obj).lon, 2)
            }
            for obj in PLANETS if chart.get(obj) is not None
        }
